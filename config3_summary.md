# Configuration 3: Normal-Condition Consistency Experiments
...
## Results (`normal_config3_experiment.py`, 1,000 trials per model)
...
# Configuration 3: Node-Failure Consistency Experiments

## Setup (`node_failure_config3.py` / `node_failure_config3_experiment.py`)
...
# Configuration 3: Network-Partition Consistency Experiments

## Setup (`network_partition_config3.py` / `network_partition_config3_experiment.py`)
...
Configuration: config3.py. Four test functions: normal_config3_experiment.py. Each fault has its own configuration and experiment entry point. config3_helpers.py provides shared logging, worker communication, waits, phase execution and recovery. Source is copied to the client container before execution; Desktop bind mounting is not required.

```bash
DOCKER_CONTEXT=colima-consistency-lab docker-compose -f compose.yaml -f compose.config3.yaml up -d --build client-config3
.venv/bin/python setup_config3.py
.venv/bin/python normal_config3_experiment.py
.venv/bin/python node_failure_config3_experiment.py
.venv/bin/python network_partition_config3_experiment.py
.venv/bin/python audit_config3.py results_config3/<run-directory>
```

These commands assume the existing rs0 is already initialized. For a fresh project deployment, start the base compose services and run the existing setup_majority.py bootstrap before configuring Config 3 tags.
...
- normal: `results_config3/20260923T083607Z-normal-formal-7d0a6c`
- node: `results_config3/20260923T083624Z-node-formal-6164b0`
- partition: `results_config3/20260923T090231Z-partition-formal-fae952`

Each directory contains the manifest, source snapshots, history.jsonl, CSV/JSON summaries, audit.json, representative examples and database logs. Root-level normal_config3_results.json, node_failure_config3_results.json and network_partition_config3_results.json point to their formal run directories.
