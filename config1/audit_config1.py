"""Audit Config 1 settings, absence of causal sessions and measured predicates."""
import hashlib
import json
import sys
...
def audit(path):
    ...
    database_name=manifest.get('database_name','dsa5208_config1')
    counts=defaultdict(Counter);errors=Counter();stages=Counter();examples={};ids=set()
    roles={};barriers=0;last_roles=None;command_counts=Counter();non_causal_reads=0;faults=[]
    for line in (path/'history.jsonl').open():
        ...
        assert r['causal_consistency'] is False and r['session_id'] is None
        ...
        for e in r['events']:
            if e['event']=='started':
                ...
                node=e['node'][0]
                if e['command']=='update':
                    assert cmd['writeConcern']['w']==1
                    assert roles[r['phase']][node]=='PRIMARY'
                elif e['command']=='find':
                    assert cmd['readConcern']['level']=='local'
                    assert 'afterClusterTime' not in cmd['readConcern'],'Unexpected causal lower bound'
                    non_causal_reads+=1
    ...
    result={'audit_passed':True,'scenario':manifest['scenario'],'trials':len(ids),
            ...
            'application_commands':dict(command_counts),'verified_reads_without_causal_bound':non_causal_reads,
