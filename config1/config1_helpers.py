...
from config1 import CONFIG, DB_NAME, NODES, create_client, create_collections, trial_session
...
class DatabaseWorker:
    ...
    def dispatch(self,q):
        ...
        if action=='barrier':
            ...
            # Recovery barrier is infrastructure, not an application operation: keep it majority-written.
            w=create_collections(self.client,name)[0].with_options(write_concern=WriteConcern(w='majority',wtimeout=2000))
    ...
    def invoke(self,label,fn):
        ...
        try:
            ...
            self.trial['operations'].append({'stage':label,'ok':True,'elapsed_ms':(time.monotonic()-start)*1000,
                                            'value':value,'operationTime':stamp,'session_operation_time':self.session.operation_time if self.session else None})
            return value,stamp
    ...
    def case(self,model,index):
        self.monitor.events=[]
        self.session=trial_session(self.client)
        cid=f'{self.phase}-{model}-{index}'
        self.trial={'case':cid,'phase':self.phase,'model':model,'trial':index,
                    'outcome':'inconclusive','operations':[], 'session_id':self.session.session_id if self.session else None, 'causal_consistency':bool(self.session and self.session.options.causal_consistency)}
...
def compose_args(*args):
    ...
    return [executable,'-f',str(ROOT.parent/'compose.yaml'),'-f',str(ROOT/'compose.config1.yaml')]+list(args),env

class Worker:
    def __init__(self):
        lookup,env=compose_args('ps','-q','client-config1')
        cid=subprocess.run(lookup,env=env,text=True,capture_output=True,check=True).stdout.strip()
        if not cid:raise RuntimeError('Start client-config1 before running experiments')
        for path in ROOT.glob('*config1*.py'):
            docker('cp',str(path),cid+':/lab/'+path.name)
        args,env=compose_args('exec','-T','client-config1','python','-u','config1_helpers.py','--worker')
...
class Run:
    def __init__(self,scenario,trials,smoke=False,models=MODELS):
        self.lock=(ROOT/'.config1.lock').open('w');fcntl.flock(self.lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
        self.started=datetime.now(timezone.utc).isoformat()
        self.id=datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'-'+scenario+('-smoke-' if smoke else '-formal-')+uuid.uuid4().hex[:6]
        self.out=ROOT/'results_config1'/self.id;self.out.mkdir(parents=True)
        self.scenario=scenario;self.trials=trials;self.smoke=smoke;self.models=models
        files=sorted(set(ROOT.glob('*config1*.py'))|{ROOT/'config1.py',ROOT/'Dockerfile.config1',ROOT/'compose.config1.yaml',ROOT.parent/'compose.yaml',ROOT/'config1_plan.md'})
    ...
    def finish(self,completed):
        try:
            ...
            if completed:
                ...
                if not self.smoke:
                    name={'normal':'normal_config1_results.json','node':'node_failure_config1_results.json','partition':'network_partition_config1_results.json','ryw':'test_ryw_config1_results.json'}[self.scenario]
...
def run_scenario(scenario,default_trials,smoke=False,models=MODELS):
    ...
    try:
        ...
        if scenario in ('normal','ryw'):run.phase('normal')
            ...
            from node_failure_config1 import stop_node,start_node
            try:
            ...
            from network_partition_config1 import partition_node,wait_for_election
            try:
...
if __name__=='__main__':
    if '--worker' in sys.argv:
        sys.modules['config1_helpers']=sys.modules[__name__]
        worker_main()
