"""Configuration 1: normal operation, 1000 trials per consistency model.

Same test functions as Configuration 2; only config1.py settings differ."""
NUM_TESTS = 1000


def test_read_your_writes(c):
    # W1 has been acknowledged (w=1). R1 reads the tagged secondary with no session.
...
def test_monotonic_reads(c):
    from config1_helpers import Inconclusive
