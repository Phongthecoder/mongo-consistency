"""Configuration 2 partition settings and election/reconnection helpers."""
import time
from config2 import CONFIG
from config2_helpers import docker, project_network
...
def wait_for_election(worker, timeout=45):
    ...
    while time.monotonic() < deadline:
        ...
        if roles['mongo1'] == 'UNREACHABLE' and 'PRIMARY' in (roles['mongo2'], roles['mongo3']):
            return roles
        time.sleep(.5)
    raise RuntimeError('Partition or majority-side election was not verified')
